...
{
    fp_pos = fopen(("traj_p.txt").c_str(), "w+");
    fp_vel = fopen(("traj_v.txt").c_str(), "w+");
    fp_acc = fopen(("traj_a.txt").c_str(), "w+");
}
...
int main(int argc, char **argv)
{
    ...
    MatrixXd waypoints(3, waypGen.N+1);     // pva * time
    waypoints = waypGen.read();

    // allocate time for waypoints
    VectorXd ts(waypGen.N+1);
    ts = allocateTime(waypoints, 12.0, 5.0);
    ...
    jerkOpt.reset(iS, fS, waypGen.N);
    // generate trajectory
    jerkOpt.generate(waypoints.block(0, 1, 3, i - 1), ts);///////////
